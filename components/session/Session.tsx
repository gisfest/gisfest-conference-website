...
const Session = (session: TSession['session']) => {
	...
	const sessionLink = `${process.env.NEXT_PUBLIC_FRONTEND_URL}${currentPath}`;
	const { onCopy } = useClipboard(sessionLink);
	...
	return (
		<section>
			<div className=" laptop:space-y-8 space-y-6">
				...
				<h2 className="laptop:text-h2 text-h5 font-p-semibold w-[90%]">
					{session.title}
				</h2>
			</div>
			<div className="laptop:space-y-12 space-y-4 laptop:mt-[215px] mt-24">
				<h4 className="laptop:text-h2 tablet:text-h3 text-h4 font-clashDisplay">
					About the Session
				</h4>
				<p className=" laptop:text-h5 text-body font-p-regular">
					{session.abstract}
				</p>
			</div>
			{session.category !== AgendaCategory.network && (
				<div>
					<div className="grid laptop:grid-cols-2 tablet:grid-cols-2 grid-cols-1 gap-14 laptop:gap-0  justify-between mt-[128px] ">
						<h4 className="laptop:text-h3 laptop:text-start tablet:text-start col-span-1 text-h4 text-center font-p-semibold laptop:order-first tablet:order-first order-last font-clashDisplay">
							Speaker(s)
						</h4>
						<div className="flex laptop:justify-end tablet:justify-end justify-start gap-4 col-span-1 items-center">
							<span className="text-body font-p-regular text-tc-10">
								Share session
							</span>
							<div
								className="rounded-full h-[39.81px] w-[39.81px] flex items-center justify-center bg-sc-20 cursor-pointer transition-all ease-in-out hover:bg-sc-10"
								onClick={handleCopy}
							>
								<TablerCopy />
							</div>
